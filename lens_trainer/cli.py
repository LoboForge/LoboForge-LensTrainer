...
def format_run_summary(cfg) -> str:
    ...
    lines = [
        ...
        f"  resolution   {cfg.dataset.resolution} ({'auto aspect' if cfg.dataset.resolution <= 0 else 'square'})",
        f"  sample_size  {cfg.sample.width}×{cfg.sample.height} ({'from dataset' if cfg.sample.width <= 0 or cfg.sample.height <= 0 else 'fixed'})",
